...
const pseudoMap = {
    'Selection Sort': 'for i in 0..n-1:\n  minIndex = i\n  for j in i+1..n:\n    if A[j] < A[minIndex]: minIndex = j\n  swap A[i], A[minIndex]',
    'Insertion Sort': 'for i in 1..n-1:\n  key = A[i]\n  j = i-1\n  while j>=0 and A[j] > key:\n    A[j+1] = A[j]\n    j--\n  A[j+1] = key',
    'Quick Sort': 'quickSort(A, low, high):\n  if low < high:\n    pi = partition(A, low, high)\n    quickSort(A, low, pi-1)\n    quickSort(A, pi+1, high)',
    'Merge Sort': 'mergeSort(A, l, r):\n  if l < r:\n    m = (l + r)//2\n    mergeSort(A, l, m)\n    mergeSort(A, m+1, r)\n    merge(A, l, m, r)',
    'Heap Sort': 'heapSort(A):\n  buildMaxHeap(A)\n  for i=n-1..1:\n    swap A[0], A[i]\n    heapify(A, 0, i)',
    'Radix Sort': 'radixSort(A):\n  maxVal = max(A)\n  exp = 1\n  while maxVal/exp > 0:\n    countingSort(A, exp)\n    exp *= 10',
    'Shell Sort': 'shellSort(A):\n  gap = n/2\n  while gap > 0:\n    for i=gap..n-1:\n      temp = A[i]\n      j = i\n      while j>=gap and A[j-gap]>temp:\n        A[j] = A[j-gap]\n        j -= gap\n      A[j] = temp\n    gap /= 2',
    'Bubble Sort': 'bubbleSort(A):\n  for i=0..n-1:\n    for j=0..n-i-2:\n      if A[j]>A[j+1]: swap A[j], A[j+1]',
    'Cocktail Shaker Sort': 'cocktailShakerSort(A):\n  swapped = true\n  start = 0\n  end = n-1\n  while swapped:\n    swapped = false\n    for i=start..end-1:\n      if A[i] > A[i+1]: swap\n    end--\n    for i=end-1..start:\n      if A[i] > A[i+1]: swap\n    start++',
    'Gnome Sort': 'gnomeSort(A):\n  i = 0\n  while i < n:\n    if i==0 or A[i] >= A[i-1]: i++\n    else:\n      swap A[i], A[i-1]\n      i--',
};
...
function createBlocks() {
    ...
    const blockWidth = 4;
    const gap = 1;
    const startX = 50 + -((blockWidth + gap) * dataset.length) / 2 + (blockWidth + gap) / 2;
...
/** Quick Sort */
async function doQuickSort(low = 0, high = dataset.length - 1) {
    if (interruptRequested)
        return;
    if (!isSorting) {
...
async function countingSortDigit(exp) {
    if (interruptRequested)
        return;
    const n = dataset.length;
...
async function doCocktailShakerSort() {
    if (isSorting)
        return;
    isSorting = true;
    interruptRequested = false;
